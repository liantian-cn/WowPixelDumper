...
class NodeTitleManager:
    ...
    def _init_database(self) -> None:
        ...
        # 如果存在旧表，迁移数据
        if 'full_blob' in columns or 'middle_blob' in columns:
    ...
    def _migrate_from_old_schema(self, conn: sqlite3.Connection, cursor: sqlite3.Cursor) -> None:
        ...
        for row in cursor.fetchall():
            ...
            cursor.execute('''
                INSERT INTO node_titles (id, full_data, middle_hash, title, match_type, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (id_, full_data, middle_hash, title, match_type, created_at))

        # 删除旧表
        cursor.execute('DROP TABLE node_titles_old')
        print('[NodeTitleManager] 数据迁移完成')
    ...
    def add_title(
        ...
        try:
            cursor.execute('''
                ...
                (full_data, middle_hash, title, match_type)
                VALUES (?, ?, ?, ?)
            ''', (full_data, middle_hash, title, match_type))
            ...
            print(f'[NodeTitleManager] 添加记录: {title} (type: {match_type})')
            return new_id
    ...
    def get_all_titles(self) -> list[TitleRecord]:
        ...
        cursor.execute('''
            SELECT id, full_data, middle_hash, title, match_type, created_at
            FROM node_titles
            ORDER BY id DESC
        ''')
