"""图标库管理对话框 - 4个Tab：图标库管理、未匹配图标、相似度匹配记录、设置。"""
...
class IconLibraryDialog(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        # Tab 1: 图标库管理
        self.tab_database: QWidget = self._create_database_tab()
        self.tab_widget.addTab(self.tab_database, '图标库管理')
    ...
    def _create_database_tab(self) -> QWidget:
        ...
        # 表格
        self.db_table: QTableWidget = QTableWidget()
        self.db_table.setColumnCount(5)
        self.db_table.setHorizontalHeaderLabels(['图标', '标题', 'Hash', '类型', '操作'])
        ...
        self.db_table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.Fixed)
        ...
        # 设置委托
        self.db_table.setItemDelegateForColumn(2, HashDisplayDelegate(self.db_table))

        layout.addWidget(self.db_table)

        # 刷新按钮
        self.refresh_db_btn: QPushButton = QPushButton('刷新列表')
        self.refresh_db_btn.clicked.connect(self.refresh_database_tab)
    ...
    def refresh_database_tab(self) -> None:
        """刷新图标库表格。"""
        records: list[TitleRecord] = self.title_manager.get_all_titles()
        self._populate_db_table(records)
        self.update_stats()

    def _populate_db_table(self, records: list[TitleRecord]) -> None:
        """填充数据库表格。"""
        ...
        for row, record in enumerate(records):
            ...
            self.db_table.setItem(row, 0, icon_item)
            ...
            self.db_table.setItem(row, 1, title_item)
            ...
            self.db_table.setItem(row, 2, hash_item)
            ...
            self.db_table.setItem(row, 3, type_item)
            ...
            self.db_table.setCellWidget(row, 4, operation_widget)
    ...
    def on_search_changed(self, text: str) -> None:
        ...
        # 过滤记录
        all_records = self.title_manager.get_all_titles()
        filtered: list[TitleRecord] = [r for r in all_records if text.lower() in r.title.lower()]
    ...
    def _create_unmatched_tab(self) -> QWidget:
        ...
        self.unmatched_table.horizontalHeader().setSectionResizeMode(5, QHeaderView.ResizeMode.Fixed)
        self.unmatched_table.setColumnWidth(0, 50)
        self.unmatched_table.setColumnWidth(1, 120)
        self.unmatched_table.setColumnWidth(5, 100)
    ...
    def refresh_unmatched_tab(self) -> None:
        ...
        for row in range(self.unmatched_table.rowCount()):
            ...
            if hash_item:
                ...
                if hash_value:
                    input_widget: QWidget | None = self.unmatched_table.cellWidget(row, 4)
    ...
    def on_add_unmatched_by_hash(self, hash_value: str) -> None:
        ...
        # 获取输入的标题
        title_input: QWidget | None = self.unmatched_table.cellWidget(target_row, 4)
                ...
                for row in range(self.unmatched_table.rowCount()):
                    ...
                    if hash_item:
                        ...
                        if row_hash == focused_hash:
                            new_input: QWidget | None = self.unmatched_table.cellWidget(row, 4)
